...
#define EPSILON 0.1
int THRESHOLD,Neighboorhood=900,windowRadius=35;
// #define HS 16  //Spatial bandwidth
// #define HC 32   //Color bandwidth
#define LIMIT 20  
#define ITR 1
// #define S 1  
float m=0.25;    //tradeoff
...
float kernel(int i,int j,colorSpace x){
    ...
    if(totalDistance(temp,x)<THRESHOLD)
        return 1;
    else 
        return 0;
...
int main(int argc, char** argv){
    for(THRESHOLD=500;THRESHOLD<=2000;THRESHOLD+=500){    
        ...
        sprintf(n,"FLAT %s_D_%d_m_%.2f_t_%d_f_%d.jpg",argv[1],Neighboorhood,m,THRESHOLD,numGroups);
        imwrite(n,image);
    }
